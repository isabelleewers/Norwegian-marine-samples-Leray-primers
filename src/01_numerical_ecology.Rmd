...
print(protist_table_specnumber)
...
## manually compute the number of observed species
protists_table_transposed_rarefied %>%
    t() %>%
    as.data.frame() %>%
    as_tibble() %>%
...
## workaround the rounding issue
tmp <- protists_table_transposed_rarefied * 100
...
tmp %>%
    ...
    t() %>%
    as.data.frame() %>%
    rownames_to_column(var = "samples") -> protist_table_observed_estimated_species

rm(tmp)

print(protist_table_observed_estimated_species)

protist_table_observed_estimated_species %>%
    filter(., str_detect(samples, "FD01")) -> FD01_subset
summary(FD01_subset)
protist_table_observed_estimated_species %>%
    filter(., str_detect(samples, "FD02")) -> FD02_subset
summary(FD02_subset)
protist_table_observed_estimated_species %>%
    filter(., str_detect(samples, "FD03")) -> FD03_subset
summary(FD03_subset)
protist_table_observed_estimated_species %>%
    filter(., str_detect(samples, "FD04")) -> FD04_subset
summary(FD04_subset)

combined_FD_subsets <- rbind(
  mutate(FD01_subset, category = "FD01_samples"),
  mutate(FD02_subset, category = "FD02_samples"),
  mutate(FD03_subset, category = "FD03_samples"),
  mutate(FD04_subset, category = "FD04_samples")
)
ggplot(data = combined_FD_subsets,
       aes(x = samples, y = S.obs)) +
      geom_col(color = "black", aes(fill = "Observed number of species")) +
      scale_fill_manual(values = c("orange2")) +
      geom_point(size = 3, aes(y = S.chao1, color = "Estimated number of species")) +
